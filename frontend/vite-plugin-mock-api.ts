...
function page(list: any[], total?: number) {
  return ok({ total: total ?? list.length, list, pageNum: 1, pageSize: 20 })
}

// Route handler: takes path + method, returns response data or null if not matched
function handleRoute(path: string, method: string): any {
  ...
  // Shelves
  if (path.startsWith('/v1/shelves/search') || path.startsWith('/v1/shelves?')) return page(mockShelves, 12)
  if (path === '/v1/shelves' && method === 'GET') return page(mockShelves, 12)
  if (path === '/v1/shelves' && method === 'POST') return ok(1)
  if (/^\/v1\/shelves\/\d+$/.test(path) && method === 'GET') return ok(mockShelves[0])
  if (/^\/v1\/shelves\/mine$/.test(path)) return page(mockShelves.slice(0, 3))
  if (/^\/v1\/users\/\d+\/shelves$/.test(path)) return page(mockShelves.slice(0, 3))
  if (/^\/v1\/shelves\/\d+\/timeslots$/.test(path)) return ok([])

  // Demands
  if (path.startsWith('/v1/demands/search') || path.startsWith('/v1/demands?') || path === '/v1/demands') return page(mockDemands, 8)
  if (/^\/v1\/demands\/\d+$/.test(path) && method === 'GET') return ok(mockDemands[0])
  if (path === '/v1/demands' && method === 'POST') return ok(1)

  // Orders
  if (path.startsWith('/v1/orders?') || (path === '/v1/orders' && method === 'GET')) return page(mockOrders, 6)
  if (/^\/v1\/orders\/\d+$/.test(path) && method === 'GET') return ok({ ...mockOrders[0], buyerId: 1, buyerNickname: '张三', sellerId: 2, sellerNickname: '李四', verifyCode: 'ABC123', verifyCodeExpire: '2026-07-01 18:00:00', serviceStartTime: null, serviceEndTime: null, completeTime: null, cancelTime: null, cancelReason: null, remark: '' })
  if (path === '/v1/orders' && method === 'POST') return ok(1)
  if (/^\/v1\/orders\/\d+\/pay$/.test(path)) return ok({ status: 2, verifyCode: 'XYZ789', verifyCodeExpire: '2026-07-01 18:00:00' })
  if (/^\/v1\/orders\/\d+\/(start-service|cancel|verify|confirm)$/.test(path)) return ok(null)
  ...
  if (path.startsWith('/v1/chat/messages')) return page([])

  // Stats
  if (path === '/v1/stats/overview') return ok({ totalOrders: 28, completedOrders: 22, totalIncome: 1500, totalExpense: 800, trustScore: 4.5, skillCount: 5, reviewCount: 12 })
  ...
  if (path.startsWith('/admin/')) {
    ...
    if (path.includes('/config')) return ok({})
    if (path.includes('/users')) return page([mockUser])
    if (path.includes('/orders')) return page(mockOrders)
    if (path.includes('/violations') || path.includes('/appeals') || path.includes('/campus-auths') || path.includes('/reviews')) return page([])
    return ok(null)
  }
  ...
  // Users by id
  if (/^\/v1\/users\/\d+$/.test(path)) return ok(mockUser)

  // Campus auth
  if (path === '/v1/user/campus-auth' && method === 'GET') return ok({ id: 1, authType: 'student_id', studentNo: '20240001', realName: '张三', campus: '仙林校区', college: '计算机学院', status: 2, statusDesc: '已认证', reviewRemark: null, reviewTime: null, createTime: '2026-01-15 10:00:00' })
  ...
  return {
    ...
    configureServer(server) {
      server.middlewares.use((req, res, next) => {
        ...
        // For requests with body, consume it fully first to avoid socket issues
        const hasBody = method === 'POST' || method === 'PUT' || method === 'DELETE' || method === 'PATCH'
        if (hasBody) {
          const chunks: Buffer[] = []
          req.on('data', (chunk: Buffer) => { chunks.push(chunk) })
          req.on('end', () => { sendResponse(path, method) })
          req.on('error', () => { sendResponse(path, method) })
          // In case body is empty and no events fire, use resume to trigger end
          req.resume()
        } else {
