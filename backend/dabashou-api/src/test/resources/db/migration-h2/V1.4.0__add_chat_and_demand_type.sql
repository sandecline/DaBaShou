...
-- 需求表新增类型字段
ALTER TABLE `dbs_demand` ADD COLUMN `demand_type` TINYINT NOT NULL DEFAULT 1 COMMENT '需求类型: 1-求助悬赏 2-技能置换';
...
CREATE TABLE IF NOT EXISTS `dbs_chat_session` (
    ...
    INDEX `dbs_chat_session_idx_user1` (`user1_id`),
    INDEX `dbs_chat_session_idx_user2` (`user2_id`),
...
CREATE TABLE IF NOT EXISTS `dbs_chat_message` (
    ...
    `create_time` DATETIME DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
    INDEX `dbs_chat_message_idx_session` (`session_id`),
    INDEX `dbs_chat_message_idx_sender` (`sender_id`),
    INDEX `dbs_chat_message_idx_create_time` (`create_time`),
