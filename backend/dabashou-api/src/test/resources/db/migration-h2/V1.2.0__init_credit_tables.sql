...
CREATE TABLE IF NOT EXISTS `user_trust_score_log` (
    ...
    `create_time` DATETIME DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
    INDEX `user_trust_score_log_idx_user` (`user_id`),
    INDEX `user_trust_score_log_idx_order` (`order_id`),
    INDEX `user_trust_score_log_idx_type` (`type`),
    FOREIGN KEY (`user_id`) REFERENCES `dbs_user`(`id`),
...
CREATE TABLE IF NOT EXISTS `credit_violation` (
    ...
    `create_time` DATETIME DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
    `update_time` DATETIME DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
    INDEX `credit_violation_idx_user` (`user_id`),
    INDEX `credit_violation_idx_order` (`order_id`),
    INDEX `credit_violation_idx_type` (`type`),
    FOREIGN KEY (`user_id`) REFERENCES `dbs_user`(`id`),
    FOREIGN KEY (`order_id`) REFERENCES `dbs_order`(`id`),
...
CREATE TABLE IF NOT EXISTS `credit_appeal` (
    ...
    `create_time` DATETIME DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
    `update_time` DATETIME DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
    INDEX `credit_appeal_idx_violation` (`violation_id`),
    INDEX `credit_appeal_idx_appellant` (`appellant_id`),
